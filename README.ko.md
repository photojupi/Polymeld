...
# 2. GitHub 프로젝트 폴더에서 실행 — 온보딩 위저드가 자동으로 시작됩니다
cd ~/projects/my-app
polymeld
# → 모델 선택 → GitHub 토큰 생성 안내 + 입력 → 완료!
# → GITHUB_REPO는 프로젝트 폴더에서 자동 감지됩니다
```
...
Phase 4(개발)에서 의존성이 없는 태스크들의 LLM 호출을 동시에 실행합니다:
...
| `/team` | 팀 구성 확인 |
| `/mode` | 인터랙션 모드 변경 (full-auto/semi-auto/manual) |
...
- Phase 5(리뷰)/Phase 6(QA) 수정 시에도 로컬 재커밋
...
| 메서드 | 용도 | 반환 |
|--------|------|------|
| `parseTasks()` | Phase 2 태스크 분해 | 구조화된 태스크 배열 |
| `parseReviewVerdict()` | Phase 5 코드 리뷰 | APPROVED / CHANGES_REQUESTED |
