...
cli:
  ...
  timeouts:
    claude: 900000         # Claude 15分（thinking 100%対応）
    gemini: 600000         # Gemini 10分
    codex: 900000          # Codex 15分（xhigh対応）
  max_turns:
...
node src/index.js run "実時間チャット機能の実装"

# インタラクションモード指定
node src/index.js run "채팅 기능" --mode full-auto   # デフォルト
node src/index.js run "채팅 기능" --mode semi-auto   # Phaseごとに確認
node src/index.js run "채팅 기능" --mode manual      # 手動制御
```
...
1. `polymeld.config.yaml`の`project.local_path`設定
...
│       └── zh-CN.json          # 中文(简体)
├── config/
│   ├── loader.js               # YAML設定ローダー + CLI/GitHub接続検証 + スコープチェック
...
`polymeld.config.yaml`でペルソナを追加/修正できます：
