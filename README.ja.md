...
Phase 4（開発）で依存関係のないタスクのLLM呼び出しを同時に実行します：
...
コードレビューおよびQAで失敗時の自動リトライ回数を設定します：

```yaml
pipeline:
  max_review_retries: 3    # レビュー → 修正 → 再レビュー最大反復回数
  max_qa_retries: 3        # QA失敗 → 修正 → 再検証最大反復回数
```
...
| `/team` | チーム構成確認 |
| `/mode` | インタラクションモード変更（full-auto/semi-auto/manual） |
...
- Phase 5（レビュー）/Phase 6（QA）修正時もローカルで再コミット
...
Phase 5: コードレビュー（最大 max_review_retries 回リトライ）
...
Phase 6: QA（最大 max_qa_retries 回リトライ）
  → QAがコードを検証
...
designDecisions         - 設計決定事項（以降のエージェントプロンプトに注入）
techStack               - 技術スタック
...
github.planningIssue    - GitHub プランニング Issue番号
```
...
| メソッド | 用途 | 戻り値 |
|--------|------|------|
| `parseTasks()` | Phase 2 タスク分解 | 構造化されたタスク配列 |
| `parseReviewVerdict()` | Phase 5 コードレビュー | APPROVED / CHANGES_REQUESTED |
...
├── pipeline/
│   └── orchestrator.js         # 8-Phaseパイプライン（Phase 0~7 + 並列実行 + チェックポイント）
├── workspace/
