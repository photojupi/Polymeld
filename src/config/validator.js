...
// CLI 설치/인증 + GitHub 토큰 + API key 연결 검증
...
async function checkGitHub() {
  ...
  try {
    ...
    // 2) 리포지토리 접근 + 쓰기 권한 확인 (실패 시 재입력 루프)
    let repoData;
    for (;;) {
      const repoRes = await fetch(`https://api.github.com/repos/${repo}`, { headers });
      if (repoRes.ok) {
        repoData = await repoRes.json();
        if (repoData.permissions?.push) break; // 성공 — 루프 탈출

        // 쓰기 권한 없음 — 재입력으로 해결 불가
        return { ok: false, reason: t("config.repoWriteDenied", { repo }) };
      }

      // 접근 실패 — 사용자에게 재입력 기회 제공
      console.log(chalk.yellow(`\n  ⚠️  ${t("config.repoAccessDenied", { repo })}`));
      const { retry } = await inquirer.prompt([{
        type: "confirm",
        name: "retry",
        message: t("config.repoRetryPrompt"),
        default: true,
      }]);
      if (!retry) return { ok: false, reason: t("config.repoAccessDenied", { repo }) };
