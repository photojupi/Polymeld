...
async function handleImageOnlyTask(ctx, task, agent, { branchName, baseBranch }) {
  ...
  try {
    ...
    task.images = { images: imageResult.images, text: imageResult.textResponse };

    if (imageResult.images.length > 0) {
      success = true;
      task.code = imageResult.textResponse || `[Image: ${imageResult.images.length} files generated]`;
      task.filePaths = imageResult.images.map(img => img.path);
  ...
  // 성공 시 → in-review, 실패 시 → in-progress 유지
  if (success) {
    await ctx.github.addComment(
      task.issueNumber,
